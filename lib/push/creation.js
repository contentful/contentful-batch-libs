...
export function createEntities (context, entities, destinationEntities) {
  ...
  return Promise.map(entities, (entity) => {
    ...
    return promise.then(
      partial(creationSuccessNotifier, destinationEntity ? 'update' : 'create'),
...
function createEntry (entry, space, skipContentModel, destinationEntries) {
  ...
  const destinationEntity = getDestinationEntityForSourceEntity(destinationEntries, entry.transformed)
  const promise = destinationEntity
    ? updateDestinationWithSourceData(destinationEntity, entry.transformed)
