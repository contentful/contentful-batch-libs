...
const HttpsProxyAgentMock = HttpsProxyAgent as jest.MockedClass<typeof HttpsProxyAgent>;
...
test('agentFromProxy creates https agent and removes proxy env variables', () => {
  ...
  agentFromProxy(agentParams);
  expect(HttpsProxyAgentMock.mock.calls[0][0]).toMatchObject(agentParams);
