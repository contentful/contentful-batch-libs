...
export function createEntities (context, entities, destinationEntities) {
  return Promise.map(entities, (entity) => {
    const destinationEntity = context.type === 'Role'
      ? getDestinationRoleForSourceRole(destinationEntities, entity.transformed)
      : getDestinationEntityForSourceEntity(destinationEntities, entity.transformed)
...
function createEntry (entry, space, skipContentModel, destinationEntries) {
  ...
  const promise = destinationEntry
    ...
    : createEntryInDestination(space, contentTypeId, entry.transformed)
  return promise
    .then(partial(creationSuccessNotifier, operation))
    .catch(partial(handleEntryCreationErrors, entry, space, skipContentModel,
      destinationEntries))
}
...
function createInDestination (context, sourceEntity) {
  ...
  const plainData = getPlainData(sourceEntity)
  return id && context.type !== 'Role'
    ? context.space[`create${context.type}WithId`](id, plainData)
    : context.space[`create${context.type}`](plainData)
}
...
function getDestinationRoleForSourceRole (destinationEntities, sourceEntity) {
  return find(destinationEntities, {
    name: get(sourceEntity, 'name', null)
  })
}
