...
export default function (space, destinationSpace) {
  ...
  const newSpace = omit(space, 'contentTypes', 'entries', 'assets', 'locales', 'roles', 'webhooks')
  return Promise.reduce(['contentTypes', 'entries', 'assets', 'locales', 'roles', 'webhooks'], (newSpace, type) => {
    return Promise.map(
      space[type],
