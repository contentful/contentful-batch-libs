 ...
 */
export function createEntries (context, entries, destinationEntries, noUpdate) {
  return Promise.map(entries, (entry) => createEntry(
    entry, context.space, context.skipContentModel, destinationEntries, noUpdate),
  {concurrency: 6})
}

function createEntry (entry, space, skipContentModel, destinationEntries, noUpdate) {
