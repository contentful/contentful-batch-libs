...
const bfjMock = bfj as jest.MockedObject<typeof bfj>;
...
test('displays error log well formatted', () => {
  const extendedExampleErrorLog: LogMessage[] = [
    ...exampleErrorLog,
    {
...
test('writes error log file to disk', async () => {
  ...
  expect(bfjMock.write.mock.calls).toHaveLength(1);
  expect(bfjMock.write.mock.calls[0][0]).toBe(destination);
  expect(bfjMock.write.mock.calls[0][1]).toMatchObject(exampleErrorLog);
  expect(bfjMock.write.mock.calls[0][2]).toMatchObject({
    circular: 'ignore',
    space: 2
  });
});
