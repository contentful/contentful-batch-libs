  ...
  .then((space) => {
    ...
    if (!contentModelOnly) {
      ...
      .then(partial(publishing.unpublishEntities,
        {space: space, type: 'Entry'}, sourceContent.deletedEntries))
      .delay(prePublishDelay)
      .then(partial(deletion.deleteEntities,
        {space: space, type: 'Entry'}, sourceContent.deletedEntries))

      .then(partial(publishing.unpublishEntities,
        {space: space, type: 'Asset'}, sourceContent.deletedAssets))
      .delay(prePublishDelay)
      .then(partial(deletion.deleteEntities,
        {space: space, type: 'Asset'}, sourceContent.deletedAssets))
    }
    ...
    if (!skipContentModel) {
      if (!skipLocales) {
        result = result
        .then(partial(deletion.deleteEntities,
          {space: space, type: 'Locale'}, sourceContent.deletedLocales))
      }

      result = result
      .then(partial(publishing.unpublishEntities,
        {space: space, type: 'ContentType'}, sourceContent.deletedContentTypes))
      .delay(prePublishDelay)
      .then(partial(deletion.deleteEntities,
        {space: space, type: 'ContentType'}, sourceContent.deletedContentTypes))
    ...
    if (!contentModelOnly) {
      ...
      .then((entities) => {
        return skipContentPublishing
          ? Promise.resolve([])
          : publishing.publishEntities({space: space, type: 'Asset'}, entities)
      })
      ...
      .then((entities) => {
        return skipContentPublishing
          ? Promise.resolve([])
          : publishing.publishEntities({space: space, type: 'Entry'}, entities)
      })
    }
